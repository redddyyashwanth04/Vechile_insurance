    ...
    def export_collection_as_dataframe(self, collection_name: str, database_name: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if database_name is None:
                ...
                # FIX: Access the underlying client object directly to select a different database
            ...
            # If the collection is completely empty, return an empty DataFrame safely
            if df.empty:
                print("Warning: Parsed collection is empty.")
                return df
                
            print(f"Data fetched successfully. Total rows: {len(df)}")
            
            # MongoDB default unique ID is '_id' (with an underscore)
